...
struct MatrixAddFunctor<GPUDevice, Dtype> {
  static void launch(::tensorflow::OpKernelContext* context,
    ...
    if (!d.ok()) {
      context->SetStatus(tensorflow::errors::Internal("Failed launching MatrixAdd on GPU"));
    }
...
struct MatrixAddGrad<GPUDevice, Dtype> {
  static void launch(::tensorflow::OpKernelContext* context,
    ...
    // backward<Dtype>
    // <<< cfg.block_count, cfg.thread_per_block, 0, context->eigen_gpu_device().stream() >>> (
    //   cfg,
    ...
    if (!d.ok()) {
      context->SetStatus(tensorflow::errors::Internal("Failed launching MatrixAddGrad on GPU"));
    }
